...
Shader "BeatSaber/Unlit Glow Transparent Wind"
{
    Properties
    {
        ...
        _MaskTex ("Transparency Mask (R)", 2D) = "white" {} // Added for your B&W mask
        ...
        _TipBendPower ("Tip Bend Power", Range(0.1, 8)) = 2.5 
        _MaxStaticBend ("Static Lean Amount", Range(0, 0.2)) = 0.03 
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _Tex;
            sampler2D _MaskTex; // Declared here
            float4 _Tex_ST;
            ...
            v2f vert (appdata v)
            {
                ...
                float swayAmt = (wave * 0.7 + wave2 * 0.3) * _WindStrength + gust * _GustStrength;
                ...
                o.scrPos = ComputeScreenPos(o.vertex); // Fixed: Use projection space vertex for screen position mapping
                return o;
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Sample the B&W mask texture (using red channel)
                fixed maskValue = tex2D(_MaskTex, i.uv).r;

                // Alpha cutout using the separate mask texture instead of base texture alpha
                if (maskValue < _Cutout) discard;

                // Dithered cutout mask in screen-space
